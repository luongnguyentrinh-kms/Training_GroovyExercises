...
import services.ManualEmployeeExcelService
...
// File excel local
String excelPath = "C:/Luong/Code/GroovyExercises/Data Files/ExcelData.xlsx"

// ========================== HANDLE FILE EXCEL BY APACHE POI ========================
Workbook wb = ExcelUtil.openWorkbook(excelPath)

// Read all employees from excel file
def employees = ManualEmployeeExcelService.readEmployees(wb)
println "Number of employees: ${employees.size()}"
println "Detail employess ${employees}"

// Read the exchange rate from excel file
def rate = ManualEmployeeExcelService.readExchangeRate(wb)
println "Rate: ${rate}"
...
def payload = [
		...
		employees: employees.collect { it.toJson() }
	]
	
JSONUtil.writeJsonToFile(payload, "Data Files/JsonDataManual.json")
